...
{
    //cube = std::make_unique<Cube>(wnd.getRenderer(), 0, -2.0f, -1.5f, 3);
...
void Game::Update()
{
    ...
    //cube = std::make_unique<Cube>(wnd.getRenderer(), 0, -2.0f, -1.5f, 3);
    //cube = std::make_unique<Cube>(wnd.getRenderer(), timer.Peek(), wnd.mouse.GetPosX() / 400.0f - 1.0f, -wnd.mouse.GetPosY() / 300.0f + 1.0f, 5.0f);
    ...
    while (!level_file.eof())
    {
        ...
        if (level_file.get() == wall)
        {
            ...
            level_block = std::make_unique<Cube>(wnd.getRenderer(), angle, x * SPACING - 5.0f, y * SPACING, z * SPACING);
        }
        //else // Floor (UNCOMMENT THIS BACK IN WHEN LEVEL OPENING IS HANDLED IN CONSTRUCTOR)
        //{
        //    level_block = std::make_unique<Cube>(wnd.getRenderer(), angle, x * SPACING -5.0f, y * SPACING -SPACING, z * SPACING);
        //}
    }
    level_file.close(); 
